...
% Package containing commands created for writing about complex analysis
\usepackage{./Local-packages/complex-analysis}
...
\usepackage{./Local-packages/PGF-TikZ-Chart-pics}
...
\title{Franciszek Leja \textit{Funkcje zespolone}
  \parencite{Leja-Funkcje-zespolone-Pub-2006} \\
  {\Large Błędy i~uwagi}}
...
\noindent
\Str{8--9} Aby~wyrażenie $1 / z = \bar{z} / ( z \bar{z} )$ miało sens, należy
...
\end{equation}
Ponieważ $z \bar{z}$ jest liczbą rzeczywistą, więc dzielenie przez nią
wynika z~pojęć już wprowadzonych. Gdy $z = 0$ wtedy $z \bar{z} = 0$,
...
\begin{equation}
  \label{eq:Leja-Funkcje-zespolone-06}
    ( 1 / z ) / w =
    \left( \frac{ \bar{z} }{ z \bar{z} } \right) / w =
    \frac{ \bar{z} }{ z \bar{z} } \frac{ \bar{w} }{ w \bar{w} } =
    \frac{ 1 }{ z \bar{z} } \frac{ 1 }{ w \bar{w} }
    ( \bar{z} \bar{w} ) =
    \frac{ 1 }{ z \bar{z} w \bar{w} } ( \bar{z} \bar{w} ) =
    \frac{ 1 }{ z w \bar{z} \bar{w} } ( \bar{z} \bar{w} ) =
    1 / ( z w ).
\end{equation}
...
????Zacytuj tu jakąś dobrą pozycję historyczną.????
...
tu~książce \parencite{Leja-Funkcje-zespolone-Pub-2006}. Tym problemem nie
...
\begin{equation}
  ...
  \left( \sum_{ m = 0 }^{ \infty } b_{ m } z^{ m } \right) =
  \sum_{ n = 0 }^{ \infty } ( a_{ n } b_{ 0 } + a_{ n - 1 } b_{ 1 } + \ldots +
  a_{ 0 } b_{ n } ) z^{ n }.
\end{equation}
...
\begin{equation}
  ...
  \absOne{ C_{ n } - A B } \leq \varepsilon.
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    C_{ 0 } + C_{ 1 } + \ldots + C_{ n - 1 } + C_{ n }
    &= n A B + A \sum_{ i = 0 }^{ n - 1 } \beta_{ i } + B \sum_{ i = 0 }^{ n - 1 }
      \alpha_{ i } + R_{ n } + C_{ n } = \\
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw[axis arrow thin] (-3.85,0) -- (3.85,0);

    \node at (3.85,-0.5)  {$\Real z$};


    \draw[axis arrow thin] (0,-0.5) -- (0,3.575);

    \node at (0.7,3.4) {$\Imag z$};
    ...
    \pic at (2.75,0) {tick x axis thin 1};
    ...
    \draw[line width = 1pt, decoration = { markings,
      mark = at position 0.295 with {\arrow{Straight Barb}},
      mark = at position 0.6 with {\arrow{Straight Barb}},},
    postaction = {decorate} ]
    (-2.75,0) -- (2.75,0) arc (0:180:2.75) -- cycle;
...
i~$\bar{z}$ jako dwie zmienne niezależne.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw[axis arrow thin] (0,0) -- (3.8,0);

    \node at (3.6,-0.45) {$\Real z$};


    \draw[axis arrow thin] (0,-0.5) -- (0,3.9);

    \node at (0.7,3.7) {$\Imag z$};


    \draw[axis line thin,dashed] (-2.5,0) -- (0,0);
    ...
    \pic at (1.9,0) {tick x axis thin 2};
    ...
    \draw[line width = 1pt, decoration = { markings,
      mark = at position 0.3 with {\arrow{Straight Barb}},
      mark = at position 0.75 with {\arrow{Straight Barb}},},
    postaction = {decorate} ]
    (1.9,0) arc (0:53:1.9) -- (53:3.5);
...
\noindent
\StrWierszDol{101}{1} Aby~wzór w~tej linii miał sens, należy zdefiniować
symbol Newtona, dla pary $\mu$, $n$, gdzie $\mu$ jest dowolną liczbą zespoloną,
$n$ liczbą naturalną. Wydaje mi~się, że~poprawna definicja~to
\begin{equation}
...
\StrWierszGora{28}{5} \\
\Jest nieograniczone \\
\PowinnoByc nieograniczone i~domknięte \\
\StrWierszGora{36}{5} \\
\Jest Ze~wzorów (9)~otrzymujemy \\
\PowinnoByc Otrzymujemy z~nich wzory~(9) i~na ich mocy mamy \\
\StrWierszGora{56}{2} \\
\Jest a~istnieją wartości~$w$, które \\
\PowinnoByc a~istnieje wartość~$w$, którą \\
\StrWierszGora{75}{12} \\
\Jest rozciętej wzdłuż jednego południka na~pas nieograniczony
płaszczyzny~$p$ nawiniętej na~powierzchnię boczną \\
\PowinnoByc rzutowanej wzdłuż południków na~pas nieograniczony~$p$ zwinięty
w~powierzchnię boczną \\
\StrWierszDol{92}{6} \\
\Jest \textit{krzywą} \\
\PowinnoByc \textit{krzywą kawałkami gładką} \\
\StrWierszGora{95}{5} \\
\Jest linią \\
\PowinnoByc dowolna linia łamana \\
\StrWierszDol{107}{12} \\
\Jest dowolnym konturem zawartym \\
\PowinnoByc dowolną drogą całkowania zawartą \\
\StrWierszDol{108}{13} \\
\Jest domkniętym \\
\PowinnoByc domkniętym i~ograniczonym \\
\StrWierszDol{119}{8} \\
\Jest \textit{są~identyczne, jeżeli~są równe na~brzegu~$B$} \\
\PowinnoByc \textit{jeśli~są równe na~brzegu $B$, to~są równe również
  w~obszarze~$D$} \\
\StrWierszDol{134}{16} \\
\Jest krzywych zamkniętych \\
\PowinnoByc kawałkami gładkich zamkniętych krzywych \\
\StrWierszGora{165}{2} \\
\Jest niech brzeg tego obszaru zawiera odcinek \\
\PowinnoByc niech część wspólna brzegu tego obszaru i~osi
$\im z = 0$ tworzy odcinek \\
\StrWierszDol{165}{6} \\
\Jest \textit{symetrii~~~Riemanna-Schwarza} \\
\PowinnoByc \textit{symetrii Riemanna-Schwarza}
