...
% ##############################
\Main{Książki Laurenta Schwartza --~błędy
  i~uwagi} % Tytuł całego tekstu
...
$f : E \to \vec{ F }$, gdzie $E$ jest zbiorem na którym można
...
zauważyć następujący fakt: jeśli $f : E \to F$ jest bijekcją
...
$\int || \vec{ f } - \vec{ f }_{ n } || \to 0$ wydaje się
...
\begin{center}
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    165 & &  5 & $\big{ | } \big( ( u + \sigma I ) \vec{ x } | \vec{ x } \big) + i \tau || \vec{ x } ||^{ 2 }$
           & $\big{ | } \big( ( u + \sigma I ) \vec{ x } | \vec{ x } \big) + i \tau || \vec{ x } ||^{ 2 } \big{ | }$ \\
    169 &  5 & & $u_{ k }( B )$ & $u( B )$ \\
