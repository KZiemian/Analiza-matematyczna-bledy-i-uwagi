...
\begin{proof}
  ...
  \begin{equation}
    ...
    \frac{ d g( x ) }{ dx } =
    \frac{ d }{ dx } \big( F_{ 1 }( x ) - F_{ 2 }( x ) \big)
...
\Work{ % Autor i tytuł dzieła
  Krzysztof Maurin \\
  \textit{Analiza. Część~I: Elementy}, \cite{MaurinAnalizaETCVolI1974}}
...
\Work{ % Autor i tytuł dzieła
  Krzysztof Maurin \\
  \textit{Analiza. Część II: Ogólne struktury, funkcje algebraiczne,} \\
  \textit{całkowanie, analiza tensorowa}, \cite{}}
...
\Work{ % Autor i tytuł dzieła
  Walter Rudin \\
  \textit{Analiza rzeczywista i~zespolona},
...
\Work{ % Autor i tytuł dzieła
  P. J. Nahin \\
  \textit{Inside Interesting Integrals},
