...
% ############################
\Main{Analiza matematyczna --~błędy i~uwagi} % Tytuł całego tekstu
...
z~reguł mnożenia. Teraz możemy \emph{zdefiniować} liczbę $1 / z$ wzorem
\begin{equation}
  \label{eq:Leja-3}
  \begin{split}
...
\start \Str{} W~przedstawiony tu rozważaniach przyjęto milcząco,
że~każda z~badanych krzywych jest \tb{regularna}, czyli pozbawiona
takich punktów, iż $z'( t ) = 0$. Noszą one nazwę \tb{punktów
  osobliwych}. Jeśli rozważana krzywa nie jest regularna, to może~się
...
i~stwierdzenie, iż~???~jest nieprawdziwe.

Powód tego jest dość oczywisty. Jeśli krzywa ma~punkty osobliwe, to
istnieje możliwość, iż~mimo zmieniającego~się parametru, będzie ona
,,stała w~miejscu''. Przykładem tego jest
\begin{equation}
  \label{eq:Leja-24}
  \begin{split}
    z( t ) =
...
w~punkcie $z = 0 + i 0$. Jeśli obliczymy ????
\begin{equation}
  \label{eq:Leja-25}
  \begin{split}
    s( t ) = \Int_{ 0 }^{ t } \abso{ z'( u ) } \, \de u
    = \Int_{ 0 }^{ t } 0 \, \de u = 0, \quad t \geq 0.
  \end{split}
\end{equation}
Widzimy więc, że~krzywej tej nie da~się sparametryzować za~pomocą zmiennej~$s$, czyli jej długości.

Jednak sama obecność punktów osobliwych, nie oznacza, że~krzywej nie da~się sparametryzować w~sposób naturalny????. Jest to wciąż możliwe, jeśli tych punktów jest ,,odpowiednio mało''. Przykładem takiej sytuacji jest krzywa
\begin{equation}
  \label{eq:Leja-26}
  \begin{split}
    z( t ) = t^{ 3 } + i 0.
