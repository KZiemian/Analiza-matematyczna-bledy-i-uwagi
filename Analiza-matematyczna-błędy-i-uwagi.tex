...
\end{equation}
można udowodnić na~dwa sposoby. W~obu przypadkach będziemy potrzebowali
następującego twierdzenia (zob. str.~??, \cite{}). Jeśli w~przestrzeni
metrycznej dane~są dwa nieprzecinające~się zbiory $C$ i~$K$, przy czym
$C$ jest domknięty, a~$K$ zwarty, to infimum odległość między punktami
tych zbiorów jest silnie większa od~0. ???? Dodatkowo jeśli zbiór $C$
jest zwarty, to~odległość ta~osiąga wartość minimalną.

Stosując to twierdzenie, dla~obrazu krzywej~$C$ i~$\set{ z }$ widzimy,
że~odległość między tymi zbiorami $L > 0$.
...
obszaru ograniczonego przez $C$ i~$K$ ($K$ przyjmujemy jako tak mały
by~zawierał~się wewnątrz~$C$\footnote{Czy pojęcie ,,znajdowania~się
  wewnątrz konturu'' zostało już zdefiniowana. Topologiczne wnętrze
  konturu zwykle jest puste, a~najpewniej zawsze musi być puste.}),
więc~całki po~nich~są równe, a~stąd wynika wzór~(17).
