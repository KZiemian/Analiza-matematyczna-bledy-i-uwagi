...
z~reguł mnożenia. Teraz możemy \tb{zdefiniować} liczbę $1 / z$ wzorem
\begin{equation}
  \label{eq:3}
  \begin{split}
...
badania rozwiązań równania \tb{trzeciego} stopnia, nie zaś równań
...
\begin{equation}
  \label{eq:6}
  \begin{split}
    \cos \theta_{ 0 } = \cos \tfr{ \vp }{ 2 } = \onehalf ( 1 + \cos \vp ),
  \end{split}
\end{equation}
podobnie dla~funkcji $\sin$. Drugi pierwiastek obliczamy za~pomocą wzorów
\begin{equation}
  \label{eq:7}
  \begin{split}
    \cos( \tfr{ \vp }{ 2 } + \pi ) = -\cos( \tfr{ \vp }{ 2 } ),
    \quad \sin( \tfr{ \vp }{ 2 } + \pi ) = -\sin( \tfr{ \vp }{ 2 } ).
  \end{split}
\end{equation}
Tym samym możemy wyrazić oba szukane pierwiastki w~prosty sposób za~pomocą $\al$ i~$\be$.
