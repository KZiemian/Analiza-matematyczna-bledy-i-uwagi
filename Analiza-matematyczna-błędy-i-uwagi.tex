...
Zacznijmy od~uwagi ogólnej. Jeśli oznaczymy $z_{ 0 } = x_{ 0 } + i y_{ 0 }$,
...
Ze~wzoru (18) na~stronie~72 wiemy, że~jeśli $f'( z_{ 0 } ) \neq 0$, to
macierz Jacobiego odwzorowania $( u( x, y ), v( x, y ) )$ jest w tym
punkcie nieosobliwa. Na~mocy twierdzenia o~lokalnym odwracaniu
odwzorowań istnieje otoczenie punktu $z_{ 0 } \in \mathcal{V}$ takie,
że~na $f( \mathcal{V} )$ istnieje odwzorowanie $f^{ -1 }$, które jest
tak gładkie jak funkcje $u( x, y )$ i~$v( x, y )$. Na~podstawie tego
oraz równań Cauchy'ego-Riemanna można pokazać, że~na zbiorze
$\mathcal{V}$ sytuacja musi wyglądać tak, jak na rysunku~(36).
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=1.3]
    ...
    \draw [line width=1pt, decoration={ markings, mark=at position 0.2
      with {\arrow[line width=1.2pt]{>}}, mark=at position 0.6 with
      {\arrow[line width=1.2pt]{>}}}, postaction={decorate}] (1, 0)
    arc (0:55:1) -- (0.57 * \R, 0.82 * \R);
